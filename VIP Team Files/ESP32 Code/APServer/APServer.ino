...
// Depth readings (When we do this for real we will probably write to a file on the sd card or something)
float depth_readings[500];
int depth_index = 0;
...
void setup(void) {
  ...
  // testingMode is true if the dip switch is pulled down to ground
  //testingMode = (digitalRead(DIP[0]) == LOW);
  testingMode=true;
  ...
  // DIP0 is set to testing mode, enable wifi and other testing services
  } else {
    Serial.begin(115200);
    ...
    if (!MDNS.begin("microfloat")) { //http://microfloat.local
    ...
    /*handle returning depth readings*/
    server.on("/depths", HTTP_GET, []() {
      server.sendHeader("Connection", "close");
      server.send(200, "text/plain", readings_to_string(depth_readings, depth_index));
    });
    server.begin();
  }
}
...
void loop(void) {
  ...
  // DIP0 is set to high efficiency mode
  if (!testingMode) {
    // We don't have nontesting/high efficiency code yet
  ...
  // DIP0 is set to testing mode, run wifi and other testing services
    ...
    // Generate sinusoid stand-in depth readings from 0 to 500m (one time only)
    if (depth_index < 500) {
      depth_readings[depth_index] = 250.0 - 250.0*cos(((float) depth_index)/10.0);
      depth_index++;
    }
  }
}
